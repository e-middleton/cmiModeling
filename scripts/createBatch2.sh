...
for depth in ${depthValues[@]}; do
    ...
    for fault in ${faultWeights1[@]}; do 
        for cmi in ${cmiWeights1[@]}; do
            if [ -d "./_outputs/batch2/D"$depth"_SU_Testing/test$i" ] ; then 
                echo "test directory already exists, not recreating"
            else
                echo "creating test directory"
                cd _outputs/batch2/D$depth"_SU_Testing"
                mkdir "test$i"
                cd test$i
                mkdir numpy
                mkdir images
            ...
            python main.py "--planeDepth=$depth" "--testName=test$i" "--faultSmoothing=$fault" "--cmiSmoothing=$cmi" "--gpsFile=./data/cumulative_disp.txt" "--outputDir=_outputs/batch2/D"$depth"_SU_Testing/test"$i
...
for depth in ${depthValues2[@]}; do
    ...
    for fault in ${faultWeights2[@]}; do 
        for cmi in ${cmiWeights2[@]}; do
            if [ -d "./_outputs/batch2/D"$depth"_SV_Testing/test$j" ] ; then 
                echo "test directory already exists, not recreating"
            else
                echo "creating test directory"
                cd _outputs/batch2/D$depth"_SV_Testing"
                mkdir "test$j"
                cd test$j
                mkdir numpy
                mkdir images
            ...
            # by default takes config.yaml as the config file
            python main.py "--planeDepth=$depth" "--testName=test$j" "--faultSmoothing=$fault" "--cmiSmoothing=$cmi" "--spatiallyVariable" "--gpsFile=./data/cumulative_disp.txt" "--outputDir=_outputs/batch2/D"$depth"_SV_Testing/test"$j
